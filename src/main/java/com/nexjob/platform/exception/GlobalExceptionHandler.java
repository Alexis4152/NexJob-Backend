package com.nexjob.platform.exception;
...
public class GlobalExceptionHandler {
    ...
    // Catch-all: no se expone el mensaje real de la excepcion al cliente para no filtrar
    // detalles internos (stack traces, nombres de clases, etc.); solo se registra en el log.
    @ExceptionHandler(Exception.class)
    public ResponseEntity<ApiResponse<Void>> handleGeneric(Exception ex) {
        log.error("Error no controlado", ex);
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(ApiResponse.error("Ocurrio un error inesperado. Intenta de nuevo mas tarde."));
    }
}
